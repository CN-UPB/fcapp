...
def latencyFlowsHelp (m, c, d, x):
	if m.W[x,d] == 1:
		link_lat_sum = 0.5 * sum(m.l_cap[v,w] for (v,w) in m.E) 
		flow_lat_min = min([m.l_flow[i] for i in m.F])
		if link_lat_sum <= flow_lat_min:
			return sum(m.f[c,d,v,w] * (m.l_cap[v,w] + m.l_cap[w,v]) for (v,w) in m.E) + m.Sat[c,x] * m.Proc[c] * m.p_flow[x] / m.p_node[c] <= m.l_flow[x]
		else:
			return sum(m.f[c,d,v,w] * m.Sat[c,x] * (m.l_cap[v,w] + m.l_cap[w,v]) for (v,w) in m.E) + m.Sat[c,x] * m.Proc[c] * m.p_flow[x] / m.p_node[c] <= m.l_flow[x]
	else:
		return Constraint.Skip
...
model.latencyCLCs = Constraint (model.C, model.V, rule = lambda m, c, d: sum(m.f[c,d,v,w] * (m.l_cap[v,w] + m.l_cap[w,v]) for (v,w) in m.E) + m.CLC[c,d]*m.Proc[c]*m.p_CLC/m.p_node[c] <= m.l_CLC)
# lantency constraint for CRCs
model.latencyCRCs = Constraint (model.C, model.C, rule = lambda m, c, d: sum(m.g[c,d,v,w] * (m.l_cap[v,w] + m.l_cap[w,v]) for (v,w) in m.E) + m.CRC[c,d]*m.Proc[c]*m.p_CRC/m.p_node[c] <= m.l_CRC)
