...
class Individual3b(IndividualBase):

	def __init__(self, cn):
		super(Individual3b, self).__init__()
		self.cn = cn
		self.fitness = Fitness()
		self.clc_perm = list(cn.C)
		random.shuffle(self.clc_perm)
		self.nn_minimum = random.random() * 5.0

	def __str__(self):
		return "{}, {}, {}".format(self.fitness, str(self.clc_perm), str(self.nn_minimum))
	...
	def __deepcopy__(self, memo):
		...
		result.nn_minimum = copy.deepcopy(self.nn_minimum, memo)
		return result
	...
	def mate(ind1, ind2):
		...
		child = Individual3b(ind1.cn)
		child.clc_perm = mydeap.cx_alternating_position(ind1.clc_perm, ind2.clc_perm)
		child.nn_minimum = .5 * (ind1.nn_minimum + ind2.nn_minimum)
		return child
	...
	def mutate(ind):
		...
		child.nn_minimum += random.gauss(mu=0.0, sigma=config.NN_MINIMUM_SIGMA)
		child.nn_minimum = max(0.0, child.nn_minimum)
		return child
	...
	def __eq__(self, other):
		return self.clc_perm == other.clc_perm and self.nn_minimum == other.nn_minimum
