...
# choose "es" or "ps" 
ESorPS = "ps"
convert = {"es": "es", "ps": "fcfs"} # needed due to renaming
instances_path = "Results/VCstudy_1.0-10.0"
plot_path = "plots/VCstudy/1.0-10.0"
outputlabel = "_vc_" + ESorPS
...
labels["Flex_" + convert[ESorPS]] = "GreedyFCAPA$_{" + ESorPS + "}$"
...
topologies = []
Properties = [] 
for vc in [i * 1.0 for i in range(1,7)]:
	p = "Flex_" + convert[ESorPS] + "_" + str(vc)
	Properties.append(p)
	#labels[p] = "GreedyFCAPA$_{" + ESorPS + "}$ ($n_{min}$ = " + str(vc) + ")"
	#labels[p] = "$n_{min}$ = " + str(vc)
	labels[p] = "$\\beta$ = " + str(vc)
xupper = 6010

inputfiles = []
tests = [51,53]
obj = ["LCAs","rt","Sats"]#,"RCAs"]	
...
colors = ['#FF0000','#00FF00','#0000FF','#FF00FF','#00FFFF','#800000','#008000','#000080','#808000','#800080','#008080','#808080','#FFFFFF','#444444','#FFFF00']
	...
	for t in topologies:
		for o in obj:
			...
			for tick in AX1.yaxis.get_major_ticks():
				tick.label1.set_fontsize(16)
			#AX1.legend()
			P.legend(plotlines, plotlabels, loc='best', shadow=False, prop=font_smaller, ncol=2)
			P.savefig(plot_path + '/plot_' + l[1] + '_' + l[3] + str(l[0]) + str(outputlabel) + '.pdf', bbox_inches='tight')
			#P.savefig(plot_path + '/plot_es_fcfs_' + l[1] + '_' + str(l[0]) + '_nodes.pdf', bbox_inches='tight')
			# F = P.figure(2)
			# numcols = 5
			# F.legend(plotlines, plotlabels, loc='upper left', shadow=False, fancybox=True, prop=font_smaller, ncol=numcols)
			# bb = Bbox.from_bounds(0, 0, 12.8, 6.4)
			#bb = Bbox.from_bounds(0, 0, 6.4, 3.4)
			#bb = Bbox.from_bounds(0, 0, 6.4, 4)
			#P.savefig(plot_path + '/plot_legend' + str(outputlabel) + '_' + str(numcols) + '_cols' + '.pdf', bbox_inches=bb)
