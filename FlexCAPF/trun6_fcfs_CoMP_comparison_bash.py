...
Results_path = 'Results/CoMP_comparison'
scenario = "Flex_fcfs"
...
for i in range(0,raise_iterations):
	for network in range(1, no_networks[test] + 1):
		...
		for x in Ftmp:
			num = cpg.cn.fdata[x]['connections']
			for v in cpg.cn.Wb[x]:
				nextflow = max(cpg.cn.F)+1
				cpg.cn.F.append(nextflow)
				cpg.cn.fdata[nextflow] = dict(cpg.cn.fdata[x])
				cpg.cn.fdata[nextflow]['p_flow'] /= num
				cpg.cn.W[nextflow,v] = 1
				cpg.cn.Wf[v].append(nextflow)
				cpg.cn.Wb[nextflow] = [v]
			cpg.remFlow(x)
		...
		foutmain.write(str(out_index) + " " + str(len(cpg.CRCs)) + " " + str(len(cpg.CLCs)) + " " + str(len(cpg.Satisfied)) + " " + str(trun) + " " + cpg.state \
				+ " " + str(cpg.getAverageCLCload()) + " " + str(cpg.getAverageLinkUsage()) + " " + str(len(cpg.cn.F)) + " \n")
		foutmain.close()
