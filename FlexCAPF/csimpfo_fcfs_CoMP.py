...
def loadlevel(i): # time i is provided in seconds
	...
	if i <= 3:
		return 0.9*(-1/27*pow(i,2)+1)*0.125
	elif i <= 6:
		return 0.9*(1/27*pow(i-6,2)+1/3)*0.125
	elif i <= 15:
		return 0.9*(1/243*pow(i-6,2)+1/3)*0.125
	else:
		return 0.9*(-1/243*pow(i-24,2)+1)*0.125
...
# initialize CPFlex
cpf = CPFlex(filename,evalscen="CoMP")
cpf.flexOperation = True
if rearrange == 0:
	cpf.flowRearrangement = False
elif rearrange == 1:
	cpf.flowRearrangement = True
else:
	print "Error: wrong rearrangement input!"
	exit(1)
cpf.clearFlows()
...
results_path = 'Results/sim_CoMP'
results_filename = results_path + '/simres_fcfs_' + str(network) + '_' + str(cpf.flowRearrangement) + '_' + str(seed_index) + '.dat'
