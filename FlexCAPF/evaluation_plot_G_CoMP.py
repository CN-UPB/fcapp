...
#instances_path = {"CoMP": "Results/CoMP","comparison": "Results/CoMP_comparison"}
#plot_path = "plots/CoMP"
#outputlabel = ""
instances_path = {"CoMP": "Results/CoMP_ldf","comparison": "Results/CoMP_comparison_ldf"}
plot_path = "plots_ldf/CoMP"
outputlabel = "_ldf"
...
#labels["CoMP"] = "Default CoMP scenario"
#labels["comparison"] = "Individual data flows"
labels["CoMP"] = "CoMP scenario"
...
for prop in Properties:
	inputfiles = []
	for t in tests:
		inputfiles.append(instances_path[prop] + "\_Results_test_" + str(t) + "_Flex_fcfs.dat")
		...
		while True:
			...
			try:
				if tmp[1] <> "timelimit":
					...
					r["LCAs"] = float(tmp[2])
					if prop == "comparison":
						r["Sats"] = float(tmp[3]) / float(tmp[8])
					else:
	...
	for t in topologies:
		for o in obj:
			...
			for i,l in enumerate(lines):
				...
				pl = AX1.errorbar(curr_xaxis, plotdata[l][0:len(curr_xaxis)], yerr=plotconfidence[l][0:len(curr_xaxis)], label=(l), lw=2, marker=markers[i], markersize=8, elinewidth=2,ls="--")
			...
			P.savefig(plot_path + '/plot_' + l[1] + '_' + l[3] + str(l[0]) + outputlabel + '.pdf', bbox_inches='tight')
			# F = P.figure(2)
