...
manual_input = False
instances_path = "Results_ldf/ESvsPS_lessb"
plot_path = "plots_ldf/ESvsPS_lessb"
outputlabel = "_lessb"
...
labels["Flex_es_lessb mesh"] = "GreedyFCAPA$_{es}$ (mesh)"
labels["Flex_fcfs_lessb mesh"] = "GreedyFCAPA$_{ps}$ (mesh)"
labels["Flex_es_lessb ring"] = "GreedyFCAPA$_{es}$ (ring)"
labels["Flex_fcfs_lessb ring"] = "GreedyFCAPA$_{ps}$ (ring)"
...
for n in no_nodes:
	for o in obj:
		...
		for i,l in enumerate(lines):
			...
			pl = AX1.errorbar(curr_xaxis, plotdata[l][0:len(curr_xaxis)], yerr=plotconfidence[l][0:len(curr_xaxis)], label=(l), lw=1.5, color=colors[i], marker=markers[i], markersize=8, elinewidth=2,ls="--")
			...
			plotlabels.append(labels[l[2] + " " + l[3]]) #+ " (" + str(l[0]) + " nodes)")
		...
		for tick in AX1.yaxis.get_major_ticks():
			tick.label1.set_fontsize(16)
		#AX1.legend()
		P.savefig(plot_path + '/plot_' + l[1] + '_' + str(l[0]) + str(outputlabel) + '.pdf', bbox_inches='tight')
